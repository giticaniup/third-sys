...
public class AccessTokenExpiredExceptionAspect {
    ...
    public void afterReturning(JoinPoint joinPoint, Object result) {
        ...
        if (result instanceof BaseResult) {
            ...
            try {
                if (baseResult.getErrorCode() == interfaceResponseCode.ACCESSTOKEN_EXPIRED.code) {
                    accessTokenManager.resetAppAccessToken();
                    accessTokenManager.resetCorpAccessToken();
                } else if (baseResult.getErrorCode() == interfaceResponseCode.CORPACCESSTOKEN_EXPIRED.code) {
                    accessTokenManager.resetCorpAccessToken();
                }
            } catch (Exception e) {
